...
function EditAddress() {
    ...
    const validateForm = async () => {
        ...
        if(form.Name.trim() === '' || form.Phone.trim() === '' || form.Door.trim() === '' || form.Street.trim() === '' || form.City.trim() === '' || form.Pincode.trim() === '') {
            ...
            let responseData;
            await axios.post('https://nutriipute-backend.vercel.app/editAddress',JSON.stringify(form), {
                headers: {
                    Accept: 'application/form-data',
                    'Content-Type': 'application/json',
                    'auth-token': localStorage.getItem('auth-token'),
                    'index-to-modify': `${indexToModify}`
                }
            ...
            if(responseData.success) {
                ...
                setTimeout(() => {
                    window.scrollTo(0, 0);
                    navigate("/Checkout");
                    setAddress(() => {address[indexToModify] = form; return address});
                    setIndexToModify(null);
                }, 1500);
            }
            else {
